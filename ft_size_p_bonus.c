...
/*   Updated: 2021/08/19 17:42:15 by vifernan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_size_p(char *aux, int i, int length, t_vari *select)
{
	...
	{
		if (select->plus > 0)
			select->space = ft_point_p_sp(aux, length, select, size);
		else
			select->space = ft_point_sp(aux, length, select, size);
	...
	if (select->asterisk > 0 && select->point == 0)
		size = ft_ast(aux, length, select, size);
	if (select->asterisk == 0 && select->point == 0)
		size = ft_nast_npoint(aux, length, select, size);
	select->value_i = 0;
	return (size);
}
